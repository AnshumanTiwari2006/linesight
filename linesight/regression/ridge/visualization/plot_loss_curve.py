...
def plot_loss_curve(self, display: bool = True):
    """Plot Ridge training loss (MSE + L2 penalty) over epochs."""
    ...
    ax.annotate(f"  Final: {round(losses[-1], 4)}", xy=(epochs[-1], losses[-1]),
                fontsize=9, color="#e05252")
    ax.set_xlabel("Epoch")
    ax.set_ylabel("Ridge Loss (MSE + L2)")
    ax.set_title(f"Training loss — Ridge (alpha={self.alpha})")
