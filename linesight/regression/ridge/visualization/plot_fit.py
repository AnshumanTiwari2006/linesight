...
def plot_fit(self, X, y, display: bool = True):
    """Scatter + regression line for single-feature Ridge model."""
    ...
    if X.shape[1] == 1:
        ...
        ax.plot(x_line, y_line, color="#1a6fcc", linewidth=2, label="Ridge fit")
        ax.set_xlabel(self.feature_names_in_[0])
        ax.set_ylabel("y")
        ax.set_title(f"Ridge Regression Fit (alpha={self.alpha})")
        ax.legend()
        ...
        ax.set_title(f"Ridge: Actual vs Predicted (alpha={self.alpha})")
        ax.legend()
        ax.spines[["top", "right"]].set_visible(False)
